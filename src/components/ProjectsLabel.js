...
const ProjectsLabel = ({ setProject }) => {
    const [sort,setSort] = useState(0)

    return (
        <section className="projects-label global-pd">
            <header className="projects-sort">
                <p>Projects ( {projectsObject.length} )</p>
                <div className="projects-label-button-container">
                    {
                        buttonObject.map((button, index) => (
                            <Button setProject={setProject} key={index} id={index} setSort={setSort} type={`${sort===index ? 'contained' : button.type }`} size={button.size}>
                             { button.text }
                            </Button>
                        ))
                    }
